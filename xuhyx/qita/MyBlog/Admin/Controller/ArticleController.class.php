<?php
	class ArticleController extends Controller{
		...
		public function insert(){
			...
			//添加文章发表时间
			$_POST['insert_at']=date('Y-m-d H:i:s',time());
			...
			if($res){
				...
				$this->error('文章添加失败！！','3',"./index.php?c=article&a=add");
				die();
			}
		}
		...
		public function index(){
			...
			if($where!=''){
				...
				$select=$article->order('insert_at desc')->limit("$start,5")->select();
			}
		...
		public function delete(){
			...
			if($res){
				...
				$this->error('文章删除失败！！','3',"./index.php?c=article&a=index");
				die();
			}
		...
		public function update(){
			...
			if($res){
				...
				$this->error('文章修改失败！！','3',"./index.php?c=article&a=edit&eid={$eid}&pic={$filename}");
				die();
			}
